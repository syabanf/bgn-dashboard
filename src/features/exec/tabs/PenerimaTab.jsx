...
export default function PenerimaTab({ D, navigateDrill }) {
    ...
    <div className="row-gap">
      <div className="kpi-grid">
        <KPI label="Total Penerima Manfaat Aktif" accent="navy"
          ...
          onClick={() => navigateDrill?.({ title: 'Total Penerima Manfaat Aktif', subtitle: 'Volume porsi per provinsi — terbesar dulu', sortKey: 'porsi' })}/>
        <KPI label="Satuan Pendidikan Terjangkau" accent="gold"
          value="20.118" unit=" sekolah"
          meta="SD / SMP / SMA / PAUD di 31 provinsi"
          onClick={() => navigateDrill?.({ title: 'Satuan Pendidikan Terjangkau', subtitle: 'Jumlah SPPG per provinsi — terbesar dulu', sortKey: 'sppg' })}/>
        <KPI label="Coverage terhadap Target Nasional" accent="leaf"
          value="24,3" unit="%"
          meta="Target: 82,9 juta anak — gap 62,8 juta"
          onClick={() => navigateDrill?.({ title: 'Coverage terhadap Target Nasional', subtitle: 'Compliance per provinsi — terburuk dulu', sortKey: 'comply' })}/>
        <KPI label="Rata-rata Porsi per Anak / Hari" accent="warn"
          ...
          onClick={() => navigateDrill?.({ title: 'Rata-rata Porsi per Anak / Hari', subtitle: 'Volume porsi per provinsi — terbesar dulu', sortKey: 'porsi' })}/>
      </div>
            ...
            <tbody>
              {topProvs.map((p, i) => (
                <tr key={p.code} className="drill-row"
                  onClick={() => navigateDrill?.({ title: p.name, subtitle: 'Breakdown kota/kab — klik untuk terus drill-down', sortKey: 'sppg', startAt: { province: p } })}>
